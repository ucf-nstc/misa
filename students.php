<?php $pagetitle = "Students"; require_once('includes/header.php'); ?>

<!-- People -->
<section class="container">
	<div class="row">
	<h1 class="col-md-12">Students</h1>
		<div class="col-md-4">
			<p><strong>Mikaeel Young, </strong><em>PhD student, Biomedical Sciences</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>Tyler Maxwell, </strong><em>PhD student, Chemistry</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>Ziyang Huang, </strong><em>PhD student, Chemistry</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>Ali </strong><strong>Ozcan</strong><strong>, </strong><em>PhD student, Chemistry</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>John </strong><strong>Bittman</strong><strong>, </strong><em>M.S. student, </em><em>NanoScience</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>Torus Washington, </strong><em>M.S. student, Nanoscience </em></p>
			<p>Fluorescence and Lifetime spectroscopy</p>
			<p><strong>Mikhael Soliman, </strong><em>PhD student, MSE</em></p>
			<p>Nanoscale imaging and spectroscopy</p>
			<p>Nanomaterials and plant tissues</p>
			
		</div>

		<div class="col-md-4">
			<p><strong>Briana Lee</strong>, <em>M.S. student, Nanoscience</em></p>
			<p>Nanoscale chemical and physical properties of bacteria</p>
			<p>Material transport in plant tissues</p>
			<p><strong>Stephen Smith, </strong><em>PhD student, Chemistry</em></p>
			<p>Nanomaterial synthesis and characterization</p>
			<p><strong>Jared Church, </strong><em>PhD student, Environmental Engineering</em></p>
			<p>Metal sensor development</p>
			<p><strong>Rebecca L. McLean, </strong><em> Under graduate student, Environmental Engineering</em></p>
			<p>Metal sensor development</p>
			<p><strong>Brandon M. Carpenter, </strong><em> Under graduate student</em></p>
			<p>Material transport in plant tissues</p>
			<p><strong>Parth</strong><strong> K. Patel, </strong><em> PhD student</em></p>
			<p>Ionophore sensor development</p>
			<p><strong>Courtney </strong><strong>Hulce</strong><strong>, </strong><em> M.S. student</em></p>
			<p>Ionophore sensor development</p>
		</div>

		<div class="col-md-4">
			<p><strong>Stephanie M. </strong><strong>Armas</strong><strong>, </strong><em>MS student, Chemistry </em></p>
			<p>Ionophore sensor development</p>
			<p><strong>Wynstona</strong><strong> P. Louis, </strong><em> Chemistry</em></p>
			<p>Ionophore sensor development</p>
			<p><strong>Dawn Mills, </strong><em> Chemistry</em></p>
			<p>Ionophore sensor development</p>
			<p><strong>Yasmine</strong><strong> A. </strong><strong>Abdellatif</strong><strong>, </strong><em>PhD student, Chemistry</em></p>
			<p>Fluorescence and Lifetime spectroscopy</p>
			<p><strong>Tahmina</strong> <strong>Banu</strong><strong>, </strong><em>MS student, Chemistry (graduated)</em></p>
			<p>Fluorescence and Lifetime spectroscopy</p>
		</div>
			
		</div>
	</div>
</section>

</div>

<?php require_once('includes/footer.php'); ?>